...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        for (int x = 0; x < 10; x += 1) {
            for (int y = 0; y < 10; y += 1) {
                if ((x + y) % 2 == 0) {
                    ...
                    g.fillRect(x * SQUARE_SIDE,
                            y * SQUARE_SIDE, BOARD_SIDE, BOARD_SIDE);
                } else {
        ...
        Iterator<Square> itr = Square.iterator();
        while (itr.hasNext()) {
            Square temp = itr.next();
            Piece currpiece = _board.get(temp);
            if (currpiece.equals(WHITE) || currpiece.equals(BLACK)) {
                drawQueen(g, temp, currpiece);
            } else if (currpiece.equals(SPEAR)) {
                drawSpear(g, temp, currpiece);
            }
        }
    }
    ...
    /** Stores 3 clicks done by user. */
    private Square[] clicks = new Square[3];

    /** Counter to keep track of number of clicks. */
    private int counter = 0;

    /** If we collect three clicks, we make the move on board. */
    private void checkClicks() {
        if (counter == 3) {
            counter = 0;
            _commands.offer(mv(clicks[0], clicks[1], clicks[2]).toString());
        }
    }
